...
/// 직원 목록
let employees = ["직원A", "직원B", "직원C", "아르바이트D", "아르바이트E", "아르바이트F"]
/// 기본급
let basePays: [Double] = [400, 300, 250, 1, 1, 1.5]
/// 아르바이트 직원인지 여부
let hourlys = [false, false, false, true, true, true]
/// 한달 간의 업무 누적 시간: 정규 직원의 경우 0
let timeCards: [Double] = [0, 0, 0, 120, 120, 120]
...
func isHourly(name: String) -> Bool {
...
// 아르바이트 직원의 급여를 계산한다.
func calculateHourlyPayFor(name: String, taxRate: Double) -> Double {
    let index = employees.firstIndex(of: name)!
